...
```{r}
dem_data = read_xlsx("/users/sarahbangerth/Desktop/IMC/Adult IMC Output/Data/Final/Patient demographics.xlsx")
head(dem_data)
...
type_data = dem_data %>% subset(select=c(`First LT`,Group))
fisher.test(table(type_data$`First LT`, type_data$Group),simulate.p.value=TRUE)

####################
## Time to biopsy ##
####################

kruskal.test(`Time from LT to Biopsy/Explant (evaluated) Days` ~ Group, data=dem_data)

#######################
## Immunosuppression ##
#######################

dem_data = dem_data %>% dplyr::mutate(Immunosuppression = case_when(
...
immuno = dem_data %>% subset(select=c(Immunosuppression,Group))
